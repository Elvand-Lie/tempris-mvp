...
export default function Spectrum() {
  ...
  // Client-side filtering â€” memoized to avoid stale closures
  ...
  // Sync selection when filters change â€” if selected finding is no longer in results, pick first
  ...
  const splitEdipExplanation = (text: string = '') => {
    const [summary, context] = text.split('\n\n---\nContext binding:');
    return { summary, context: context ? `Context binding:${context}` : null };
  };
    ...
    <div className="flex gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ height: 'calc(100vh - 120px)' }}>
      {error && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 bg-danger/10 border border-danger/30 text-danger px-4 py-3 rounded-lg flex items-center gap-3 shadow-lg">
          ...
          <button onClick={() => setError(null)} className="ml-4 opacity-70 hover:opacity-100 text-lg">Ã—</button>
        </div>
      )}

      {/* â”€â”€ LEFT SIDEBAR: CVE List â”€â”€ */}
          ...
          {pageFindings.map(f => (
            ...
            >
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {f.finding_type && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider bg-primary-500/10 text-primary-400 border border-primary-500/20">
                    {f.finding_type}
                  </span>
                )}
              </div>
      ...
      {/* â”€â”€ RIGHT PANEL: Detail View â”€â”€ */}
      {finding ? (
            ...
            <div className="p-6 border-b border-border bg-danger/5 flex justify-between items-start">
              <div>
                <div className="flex items-center gap-3 mb-2 flex-wrap">
                  ...
                  <h2 className="text-lg font-bold">{finding.cve || finding.id} â€” {finding.title || 'Untitled'}</h2>
                  <span className={`text-white text-xs px-2 py-0.5 rounded font-bold uppercase tracking-wider ${finding.tes_priority === 'P0' ? 'bg-danger' : finding.tes_priority === 'P1' ? 'bg-warning' : 'bg-blue-500'}`}>
                    TES {finding.tes_priority || finding.priority}
                  </span>
                  {finding.severity && (
                    <span className="text-xs px-2 py-0.5 rounded font-bold uppercase tracking-wider bg-surface border border-border text-text-muted">
                      {finding.severity.source} {finding.severity.score?.toFixed?.(1) || finding.severity.score} {finding.severity.label}
                    </span>
                  )}
                  {edipDecision && (
                    <span className={`text-xs px-2 py-0.5 rounded font-bold uppercase tracking-wider ${
                      ...
                      {edipDecision === 'mitigate' ? 'âœ“ Mitigated' :
                       edipDecision === 'accept' ? 'âœ“ Accepted' :
                       edipDecision === 'transfer' ? 'âœ“ Transferred' :
                       'âœ“ False Positive'}
                    </span>
                  )}
                </div>
                ...
                {finding.fim_bypass && (
                  <div className="mt-4 flex items-start gap-3 rounded-lg border border-danger/40 bg-danger/10 p-3 text-sm text-danger">
                    <ShieldAlert size={18} className="mt-0.5 shrink-0" />
                    <div>
                      <div className="font-bold uppercase tracking-wider">FIM Bypass</div>
                      <div className="text-danger/90">{finding.fim_bypass_note || 'Conventional file integrity monitoring will not detect exploitation. Deploy memory integrity monitoring.'}</div>
                    </div>
                  </div>
                )}
                {(finding.finding_type || finding.patch_available !== undefined || finding.compensating_controls?.length || finding.attack_vectors?.length || finding.source_references?.length) && (
                  <div className="mt-4 rounded-lg border border-border bg-surface p-3 text-sm">
                    <div className="font-bold uppercase tracking-wider text-text-muted text-xs mb-2">Final Pack Context</div>
                    <div className="flex flex-wrap gap-2 mb-2">
                      {finding.finding_type && <span className="rounded bg-primary-500/10 px-2 py-0.5 text-[10px] font-bold uppercase text-primary-400">{finding.finding_type}</span>}
                      {finding.source_detail && <span className="rounded bg-surfaceHover px-2 py-0.5 text-[10px] font-bold uppercase text-text-muted">{finding.source_detail}</span>}
                      {finding.patch_available !== undefined && <span className="rounded bg-surfaceHover px-2 py-0.5 text-[10px] font-bold uppercase text-text-muted">Patch: {finding.patch_available ? 'Available' : 'No Patch Path'}</span>}
                    </div>
                    {finding.attack_vectors?.length > 0 && <p className="text-xs text-text-muted">Vectors: {finding.attack_vectors.join(', ')}</p>}
                    {finding.compensating_controls?.length > 0 && <p className="mt-1 text-xs text-text-muted">Controls: {finding.compensating_controls.join('; ')}</p>}
                    {finding.source_references?.length > 0 && <p className="mt-1 text-xs text-text-muted">References: {finding.source_references.join('; ')}</p>}
                  </div>
                )}
              </div>
              <div className="flex flex-col items-end shrink-0">
                <span className="text-3xl font-black text-danger tracking-tighter">{finding.tes_score?.toFixed(1) || 'â€”'}</span>
                <span className="text-xs text-text-muted font-medium">TES SCORE</span>
                <span className="text-[10px] text-text-muted uppercase">{finding.tes_decision}</span>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {finding.tes_breakdown && (
                    <>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm"><Activity size={16} className="text-primary-500" /><span>Base CVSS ({finding.raw_inputs?.cvss || 0} / 10) Ã— 0.35</span></div>
                        <span className="font-mono font-medium">{finding.tes_breakdown.cvss_component?.toFixed(2)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm"><GitMerge size={16} className="text-danger" /><span>Exploitability ({finding.raw_inputs?.exploitability || 0}) Ã— 0.25</span></div>
                        <span className="font-mono font-medium">{finding.tes_breakdown.exploitability_component?.toFixed(2)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm"><Server size={16} className="text-warning" /><span>Business Impact ({finding.raw_inputs?.business_impact || 0}) Ã— 0.20</span></div>
                        <span className="font-mono font-medium">{finding.tes_breakdown.business_impact_component?.toFixed(2)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm"><Cpu size={16} className="text-primary-500" /><span>Asset Criticality ({finding.raw_inputs?.asset_criticality || 0}) Ã— 0.12</span></div>
                        <span className="font-mono font-medium">{finding.tes_breakdown.asset_criticality_component?.toFixed(2)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm"><Users size={16} className="text-text-muted" /><span>Threat Actor ({finding.raw_inputs?.threat_actor_activity || 0}) Ã— 0.08</span></div>
                  ...
                  <div className="pt-3 mt-3 border-t border-border flex items-center justify-between text-lg font-bold">
                    <span>Final TES</span>
                    <span className="text-danger">{finding.tes_score?.toFixed(2) || 'â€”'}</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className={`p-4 rounded-xl border ${
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded ${
                        ...
                        {finding.auto_classification.decision === 'fix' ? 'âš¡ AUTO-FIX' :
                         finding.auto_classification.decision === 'defer' ? 'â³ AUTO-DEFER' :
                         finding.auto_classification.decision === 'accept_candidate' ? 'âœ“ ACCEPT CANDIDATE' :
                         'ðŸ‘¤ MANUAL REVIEW'}
                      </span>
                    </div>
                    {(() => {
                      const parsed = splitEdipExplanation(finding.auto_classification.explanation);
                      return (
                        <div className="mb-3 space-y-2">
                          <p className="text-sm text-text-muted">{parsed.summary}</p>
                          {parsed.context && (
                            <div className="rounded border border-border bg-background px-3 py-2 font-mono text-[10px] leading-relaxed text-text-muted">
                              {parsed.context}
                            </div>
                          )}
                        </div>
