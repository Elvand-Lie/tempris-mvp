...
export default function SpeakWidget() {
  ...
  const clearChat = async () => {
    ...
    setHistoryLoaded(true);
    try {
      await apiDelete('/api/speak/history');
    } catch {
      setMessages([{ role: 'assistant', content: 'I could not clear the saved chat history. Please try again.' }]);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-3 rounded-2xl text-sm max-w-[280px] overflow-hidden ${msg.role === 'user' ? 'bg-surface text-text-main rounded-tr-sm' : 'bg-primary-500/10 border border-primary-500/20 text-text-main rounded-tl-sm'}`}>
                  {msg.role === 'assistant' ? (
                    <div className="prose prose-invert prose-sm max-w-none prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-strong:text-text-main prose-code:text-primary-300 prose-code:before:content-none prose-code:after:content-none">
