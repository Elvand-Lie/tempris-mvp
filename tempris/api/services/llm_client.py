...
def chat_completion(system_prompt: str, user_message: str, max_tokens: int = 500, history: list = None, user_email: str = None) -> str:
...
- Focus only on answering the user's security question using the data provided."""
    ...
    try:
        ...
        if content and len(content.strip()) > 10:
            content = filter_llm_output(content, system_prompt)
            return _append_context_binding(content, user_email)
    ...
    response = _mock_response(system_prompt, user_message)
    return _append_context_binding(response, user_email)


def _append_context_binding(response: str, user_email: str = None) -> str:
    """Append context-binding footer to AI responses.
    
    References client-specific identifiers so bulk extraction produces
    outputs tied to one client's infrastructure, not generic intel.
    """
    assets = []
    try:
        ...
        db = SessionLocal()
        rows = db.query(Asset.id, Asset.name, Asset.ip_address).filter(
            Asset.status == "active"
        ).limit(3).all()
        assets = [
            {"asset_id": row[0], "asset_name": row[1], "asset_ip": row[2]}
            for row in rows
        ]
        db.close()
    except Exception:
    ...
    return response + _build_context_binding_footer(
        account_ref=user_email or "anonymous",
