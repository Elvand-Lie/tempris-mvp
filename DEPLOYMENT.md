...
The script refuses tracked worktree changes, archives only committed Git content, verifies SHA-256 after upload, and creates verified timestamped backups of the application source, PostgreSQL database, and generated report artifacts. It preserves `.env`, mounted runtime data, and Docker volumes. Before replacing source, it applies migrations `006_add_sss_sub_class.py`, `007_create_tenant_registry.py`, `008_canonical_posture_and_operations.py`, `009_canonical_grc_framework.py`, `010_live_cve_tes_context.py`, `011_canonical_vulnerability_spine.py`, `012_finding_canonical_cve_linkage.py`, and `013_asset_scan_authorizations.py`. Migration 008 writes a JSON report under `/home/tempris/backups/migrations/`; additive migration 009 seeds the server-managed ISO/IEC 42001 catalogue, migration 010 adds CVE-context provenance without promoting legacy asset links, migration 011 creates the canonical vulnerability spine tables, migration 012 establishes exact finding linkage schema, and migration 013 implements asset scan authorizations and scan job target provenance. The release then installs the staged source and product documentation, restarts the Compose stack, checks `/api/health`, validates migrations 008 through 013 read-only, and records the deployed Git commit in `/home/tempris/app/REVISION`.
