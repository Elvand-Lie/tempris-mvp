...
async def trigger_scan(
    ...
    if target.scan_type == "full":
        # Run Nuclei and Nmap in parallel
        if NUCLEI_AVAILABLE and NMAP_AVAILABLE:
            nuclei_task = _run_nuclei_scan(target.target, scan_id)
            nmap_task = _run_nmap_scan(host_clean, scan_id)
            nuclei_results, nmap_results = await asyncio.gather(nuclei_task, nmap_task)
            all_findings.extend(nuclei_results)
            # Only add nmap port findings that weren't already found by nuclei
            nuclei_ports = {f["port"] for f in nuclei_results}
            all_findings.extend([f for f in nmap_results if f["port"] not in nuclei_ports])
        elif NUCLEI_AVAILABLE:
            all_findings = await _run_nuclei_scan(target.target, scan_id)
        elif NMAP_AVAILABLE:
            all_findings = await _run_nmap_scan(host_clean, scan_id)
        else:
    ...
    engines_used = []
    if NUCLEI_AVAILABLE and target.scan_type in ("full", "quick"):
        engines_used.append("Nuclei")
    if NMAP_AVAILABLE and target.scan_type in ("full", "ports"):
        engines_used.append("Nmap")
    if not engines_used:
        engines_used.append("Built-in TCP")
