...
| Assess business impact — SPECTRUM | Analyst/Superadmin on an open confirmed CVE exposure | `PATCH /api/spectrum/findings/{id}/business-impact` | Stores validated 0–10 analyst assessment and justification, recalculates current CVE TES server-side, writes audit/operational event, and publishes a finding refresh. It does not alter CVSS or asset confirmation. | Assess/update again with a new justification; historical score provenance is retained. |
