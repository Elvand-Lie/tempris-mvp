...
import { FileText, Shield, BookOpen, Eye, X, Upload, Trash2, Download, Paperclip, CheckCircle2, AlertTriangle, Clock } from 'lucide-react';
...
export default function GrcTes() {
  ...
  const categoryColor: Record<string, string> = {
    'AI Governance': 'bg-purple-500/20 text-purple-400 border-purple-500/30',
    'Security Operations': 'bg-red-500/20 text-red-400 border-red-500/30',
    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between pb-6 border-b border-border">
        ...
        <div className="text-right">
          <div className="flex items-center gap-1.5 text-green-500 text-[10px] font-mono font-semibold">
      ...
      <div className="flex gap-1 bg-surface p-1 rounded-xl border border-border w-fit">
        {([['tes', 'TES Dashboard'], ['grc', 'GRC SOP Builder'], ['gap', 'Gap Analysis'], ['policies', 'Policies & Frameworks']] as const).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setActiveTab(key)}
            className={`font-mono text-[11px] font-semibold tracking-wide px-4 py-2 rounded-lg uppercase transition-all ${
              activeTab === key ? 'bg-blue-600 text-white' : 'text-text-muted hover:text-text-main hover:bg-surfaceHover'
            }`}
          >
            {label}
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <div key={mod.label} className="glass-panel p-5 text-center hover:border-blue-500/30 transition-colors">
      ...
      {activeTab === 'grc' && (
        <div className="space-y-6">
          ...
          {GRC_CONTROLS.map((ctrl, i) => {
            ...
            return (
              <div key={ctrl.id} className="bg-surfaceHover border border-border rounded-xl overflow-hidden hover:border-blue-500/30 transition-colors">
                {/* Control Header */}
                <div className="p-5">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {GRC_CONTROLS.map((ctrl, i) => {
                  ...
                  const evidence = evidenceMap[ctrl.id] || [];
                  return (
                    <tr key={ctrl.id} className="border-b border-border/40 hover:bg-blue-500/[0.03]">
